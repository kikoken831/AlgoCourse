...
public class AVL<T extends Comparable<? super T>> {
    ...
    private AVLNode<T> removeHelper(AVLNode<T> root, T data, AVLNode<T> dummy) {
        if (root == null) {
    ...
    private AVLNode<T> removeSuccessor(AVLNode<T> right, AVLNode<T> dummy2) {
        if (right.getLeft() == null) {
            dummy2.setData(right.getData());
            return right.getRight();
        } else {
            right.setLeft(removeSuccessor(right.getLeft(), dummy2));
        }
